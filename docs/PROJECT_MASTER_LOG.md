...
## 2026-08-16 — Auditoría integral, sesión, contraste y calendario

Se confirmó que la sesión Google Web no puede considerarse permanente solo porque MotoCheck llame a `signInSilently()`: el plugin GIS conserva estado en memoria y el navegador puede exigir interacción si no conserva cookies o si Google revoca o renueva la autorización. El botón Web se cambió al botón oficial `renderButton` de `google_sign_in_web` para abandonar el flujo Flutter `signIn()` desaconsejado en Web. El backup seguirá siendo opcional y local-first; no se guardarán tokens en Drift ni LocalStorage.

La tarjeta de datos de la moto fue rediseñada por segunda vez después de la revisión visual del propietario. Ahora usa una superficie grafito, borde lateral naranja, texto secundario de mayor tamaño y badges semánticos con color, borde y tipografía más legibles. Se eliminaron emojis de los badges de la tarjeta porque competían con el texto y reducían la consistencia visual. La tarjeta de resumen rápido conserva su altura uniforme de 108 px.

Se creó `docs/COMPETITOR_AND_PRODUCT_AUDIT_2026-08-16.md` con fuentes públicas de MotorCheck, MotoMeteo, Detecht, Calimoto, `device_calendar` y `add_2_calendar`. La conclusión SpaceX es mantener v1 enfocada en mantenimiento, combustible, refacciones, alertas, calendario opcional y respaldo opcional; marketplace, clima/rutas, mapas, comunidad y tracking quedan fuera de v1 hasta que el núcleo sea confiable.

Se inició calendario como una proyección del mantenimiento: Drift schema v11 agrega de forma aditiva `MaintenanceRecords.calendarEventId`; al guardar un próximo servicio, la app pregunta si el usuario desea crear el evento, solicita permisos solo después de esa decisión y crea/actualiza el mismo evento mediante `device_calendar` en Android/iOS. Android declara `READ_CALENDAR`/`WRITE_CALENDAR`; iOS declara las claves de calendario para iOS 10+/17+. Web genera un `.ics` descargable con alarma de 24 horas, porque no existe un permiso de navegador equivalente para modificar directamente el calendario del dispositivo.
...
Commits recientes con alcance explícito: `57d28dd` (botón GIS Web, contraste visual e informe de competidores), `00ad426` (dependencia directa GIS Web), y el commit de calendario se creará después de cerrar CI y la validación de build. Se añadirán notas Git estructuradas a los commits del ciclo para registrar momento, cambios, pruebas y rollback.
